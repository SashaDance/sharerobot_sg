...
def render_html(rows: list[dict[str, Any]]) -> str:
    ...
    count_text = " · ".join(f"{key}: {counts.get(key, 0)}" for key in ("fully_successful", "partially_successful", "unsuccessful", "pending"))
    dataset_rows = []
...
def build_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for dataset in sorted({row["dataset"] for row in rows}):
        ...
        datasets[dataset] = {
            ...
            for status in ("fully_successful", "partially_successful", "unsuccessful", "pending")
        }
    return {
