...
class DA3_Streaming:
    ...
    def process_long_sequence(self):
        ...
        # Upstream writes per-frame artifacts only while aligning adjacent chunks.
        # A video of 30 or fewer frames is exactly one valid streaming chunk, so
        # materialize that chunk directly using the same native writers.
        if len(self.chunk_indices) == 1:
            ...
            pcd_confs = confs_first
            pcd_threshold = (
                np.mean(pcd_confs)
                * self.config["Model"]["Pointcloud_Save"]["conf_threshold_coef"]
            )
            valid_pcd = (pcd_confs >= pcd_threshold) & (pcd_confs > 1e-5)
            sample_ratio = self.config["Model"]["Pointcloud_Save"]["sample_ratio"]
            if int(np.count_nonzero(valid_pcd) * sample_ratio) == 0:
                pcd_confs = confs_first + 1.0
                pcd_threshold = (
                    np.mean(pcd_confs)
                    * self.config["Model"]["Pointcloud_Save"]["conf_threshold_coef"]
                )
    ...
    def run(self):
        ...
        def numeric_frame_key(path):
            ...
            if suffix.isdigit():
                return (0, int(suffix), path)
            return (1, 0, path)

        self.img_list = sorted(candidates, key=numeric_frame_key)
        # print(self.img_list)
    ...
    def save_camera_poses(self):
        ...
        # A single chunk has no following chunk to supply its trailing overlap.
        # Keep all of its native poses instead of leaving the last overlap_e
        # entries as None. Multi-chunk behavior remains unchanged.
        first_chunk_end = (
